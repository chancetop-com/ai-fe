...
- REST `getStatus` only on page hydrate and session switch; open PUT `events` once when `running`
- Use POST `stream` only on send (`prepareSession({ hydrate: false })`); never open PUT `events` on send or reconnect
- Drop unused recover helpers (`suppressRecoverRef`, resync path, mid-turn POST resend)
- Show `connection_disconnected` banner in `AiChat`
